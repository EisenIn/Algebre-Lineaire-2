...
\begin{theorem}
  ...
  \begin{definition}
    \label{def:49}
    Soit $A \in \mathbb{Z}^{m\times n}$ et e $rang(A)=m$. L'ensemble  $\Lambda(A):=\left\{ Ax,x\in \mathbb{Z}^{n\times n} \right\}$ est  un \emph{réseau entier généré}  de $A$. Une matrice  $B \in \mathbb{Z}^{m\times m}$ telle que  $\Lambda(A)=\Lambda(B)$ est appelée base de $\Lambda(A)$. 
  \end{definition}
...
\item Soit $U ∈ℤ^{n ×n}$. Montrer $ℤ^n = \{Ux ：x ∈ℤ^n\}$ si et seulement si $U$ est une matrice unimodulaire. 
\end{enumerate}
