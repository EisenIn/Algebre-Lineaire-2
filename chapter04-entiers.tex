...
\begin{theorem}
  ...
  \begin{definition}
    \label{def:49}
    Soit $A \in \mathbb{Z}^{m\times n}$ tq. $rang(A)=m$ alors on pose $\Lambda(A):=\left\{ Ax,x\in \mathbb{Z}^{n\times n} \right\}$ Le réseau entier de $A$. Aussi $U \in \mathbb{Z}^{n\times n}$ tq. $\Lambda(A)=\Lambda(U)$ est appelée base de $\Lambda(A)$.
     \end{definition}
