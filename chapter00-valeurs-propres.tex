...
\begin{proof}
Pour $v ∈V$ soit $[v]_B ∈K^n$ le vecteur des coordonnées de $v$ relatif à $B$. On a $[f(v_i)]_B = A \,  [v_i]_B$ pour $i\in\{1,\dots,n\}$. Si $[v_i]_B = e_i$, et $f(v_i) = λ_i v_i$ alors
    \begin{displaymath}
...
\begin{proof}
  ...
  \end{equation}
  Puisque \eqref{eq:35} est un contre exemple minimal, on  $α_i≠0$ pour tout $i$. Nous pouvons supposer que $λ_r ≠0$. Autrement, on réarrange les termes de~\eqref{eq:35}. 
...
\begin{theorem}[Théorème fondamental d'algèbre]
  \label{thr:44}
