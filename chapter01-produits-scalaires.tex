...
\begin{definition}
  ...
  \item Si $u,v$ et $w$ sont des éléments de $V$, et $α ∈ K$ est un scalaire,  \label{ps:2}
    \begin{displaymath}
      \pscal{u,v+w} = \pscal{u,v}+\pscal{u,w} \quad \text{ et } \pscal{u,α ⋅ w} = α ⋅\pscal{u,v}. 
    \end{displaymath}
  \item 
    Si $u,v$ et $w$ sont des éléments de $V$, et $α ∈ K$ est un scalaire,  \label{ps:3}
    \begin{displaymath}
      \pscal{v+w,u} = \pscal{v,u}+\pscal{w,u} \quad \text{ et } \pscal{α ⋅ u, w} = α ⋅\pscal{u,v}. 
    \end{displaymath}     
  \end{enumerate}
La  forme bilinéaire  est \emph{symétrique} si 
pour tout $v,w \in V$ 
\begin{displaymath}
    \pscal{v,w} = \pscal{w,v}. 
\end{displaymath}
\noindent
On dit que la forme bilinéaire est \emph{non dégénéré} si, de plus, la condition suivante est vérifiée:
\begin{quote}
...
\begin{eqnarray*}
  ...
  &= &\pscal{u,v} + \pscal{u,w}
\end{eqnarray*}
 et $\pscal{u,α ⋅w} = \sum_{i=1}^n u_i α w_i) = α ∑_{i=1}^n u_i w_i = α \pscal{u,w}$. 
On appelle cette forme bilinéaire la \emph{forme bilinéaire standard} de $K^n$.   On vérifie aussi très facilement que la forme bilinéaire standard est symétrique et non dégénéré. 
\end{example}
...
\begin{example}
  ...
  \end{displaymath}
Clairement, $\pscal{,}$ est une forme bilinéaire symétrique sur $V$ non dégénéré. 
\end{example}

\begin{exercise}
  \label{ex:3}
  Montrer que les  formes bilinéaires  des exemples~\ref{ex:1} et \ref{ex:2} sont non dégénérés. 
\end{exercise}
...
\begin{exercise}
  Soit $V$ de dimension fini et $B$ une base de $V$. 
...
\begin{proposition}
  \label{prop:7}
  Soit $V$ de dimension fini, $B= \{b_1, \dots , b_n\}$ une base de $V$ et $f : V × V ⟶K$ une forme bilinéaire.   Les conditions suivantes sont équivalentes. 
  \begin{enumerate}[i)]
  \item $\rank(A_B^f) = n$ \label{item:12}
  \item $f$ est non dégénéré \label{item:13}
...
\begin{framed}\noindent 
  Pour ce paragraphe~\ref{sec:orthogonalite}, si pas spécifié autrement,  $V$
...
\begin{lemma}
 ...
 Soit $\mathrm{Char}(K)\neq 2$. Si $\pscal{u,u}=0$ pour tout $u \in V$ alors 
  ...
  \end{displaymath}
On dit que la forme bilinéaire symétrique $\pscal{,}$ est  \emph{nul}. 
\end{lemma}
...
Soit $\dim(V) >1$.  Si $\pscal{u,u} = 0$ pour tout $u$, le lemme~\ref{lem:1} implique que la forme bilinéaire symmetrique est nul et toute base de $V$  est orthogonale.  
...
\end{displaymath}
alors $V = V_1 + V_1^\perp$. Cette somme est directe parce que chaque élément de $V_1 \cap V_1^\perp$ s'écrit comme $\beta \cdot u$ pour $\beta \in K$. Et $\pscal{u,\beta u} = \beta \pscal{u,u} = 0$ implique $\beta = 0$. 
...
\item Soit $K$ un corps fini. Montrer que $|K| = q^\ell$ pour un nombre prime $q$ et un nombre naturel $\ell ∈ \N$. \emph{Indication: $K$ est un espace vectoriel de dimension fini sur $\Z_q$ pour un $q$ prime.}
...
\text{ et }
 v_3 =
  ...
  \end{displaymath}
Est-ce que $\spa\{v_1,v_2,v_3\}$ possède une base orthogonale par rapport au forme bilinéaire symmetrique standard de l'exemple~\ref{ex:1}? 

\item En considérant le forme bilinéaire symmetrique standard de l'exemple~\ref{ex:1}, trouver une base orthogonale du sous-espace de $\Z_3^4$ engendré par 
  \begin{displaymath}
    v_1 =
    \begin{pmatrix}
      1\\1\\1\\0
...
\begin{framed}\noindent 
  Pour ce paragraphe~\ref{sec:class-des-matr}, si pas spécifié autrement,  $V$
  est toujours un espace vectoriel sur $K$
  muni d'une forme bilinéaire symétrique $\pscal{,}$. 
\end{framed}
...
\begin{example}
  \label{exe:22}
  Si $V$ est de dimension fini et $B,B'$ sont deux bases, \eqref{eq:28} montre que $A_B^{\pscal{,}} \cong A_{B'}^{\pscal{,}}$. 
\end{example}
...
Maintenant soit $K = \R$  et $A \in \R^{n \times n}$ symétrique. Corollaire~\ref{co:4} implique, qu'il existe une matrice inversible $P ∈ ℝ^{n \times n}$ tel que $P^T A P = D$ où $D$ est une matrice diagonale. Si on échange deux colonnes de $P$ et obtient  $P'$, alors $P'^T A P' = D'$ où $D'$ est obtenu de $D$ en échangeant les éléments diagonaux correspondants. Alors on peut trouver une matrice inversible $P ∈ ℝ^{n ×n}$ tel que 
\begin{equation}
  \label{eq:29}
  P^T A {P} =
    \begin{pmatrix}
    ...
    \end{pmatrix}. 
\end{equation}
où les  $c_i$ sont ordonnés comme  $c_1,\dots, c_r >0$, $c_{r+1},\dots ,c_s<0$ et $c_{s+1},\dots , c_n = 0$. En multipliant les premiers $s+r$ colonnes de $P$  avec $1/ \sqrt{|c_i|}$ on obtient en fait une factorisation~\eqref{eq:29} telle que 
 $c_1,\dots, c_r =1$, $c_{r+1},\dots ,c_s=-1$ et $c_{s+1},\dots , c_n = 0$. 

Alors on trouve  $P ∈ ℝ^{n ×n}$ inversible tel que 
\begin{equation}
  \label{eq:5}
  P^T A P =
  \begin{pmatrix}
...
\begin{definition}
  \label{def:37}
  Pour un espace vectoriel sur $ℝ$ de dimension fini, on appelle une base $B$ de $V$ tel que $A_B^{\pscal{,}}$ en forme~\eqref{eq:5} une \emph{base de Sylvester}. 
\end{definition}
