...
\begin{quote}
  Si $v \in V$, $v \neq 0$, alors il existe $w \in V$ tel que $\pscal{v,w} \neq 0$. 
\end{quote}
\end{definition}
...
qui est égale a $1$. Alors $f(v,b_i) \neq 0$. Donc $f$ est non dégénérée.

\ref{item:13}) $\Rightarrow$ \ref{item:12}) : On montre la contraposée. Si $\rank(A_B^f) < n$, alors il existe $x \neq 0$ tel que $[x]_B^T A_B^f = 0 \Rightarrow [x]_B^T A_B^f [y]_B^T = 0$ pour tout $y \in V$. Ceci implique que $f$ est dégénérée.
\end{proof}
...
\item Soient $V$ un espace vectoriel muni d'un produit scalaire $\pscal{.}$ et $\{v_1,\dots,v_n\} \subseteq V$ un ensemble de
...
\begin{definition}
\label{def:9}
  L'ensemble des  applications linéaires  de $V$ dans $K$ est noté $V^*$ et, muni de l'addition et de la multiplication scalaire usuelles, est appelé l'\emph{espace dual} de $V$. Les éléments de $V^*$ sont appelés \emph{formes linéaires}.
\end{definition}
...
\begin{lemma}
  \label{lem:4}
  Soit $V$
  un espace vectoriel sur un corps $K$ de dimension finie muni d'un produit scalaire
  $\pscal{.}$
