...
Dans cette section, nous fournirons une preuve du Théorème~\ref{thr:58}. À cette fin, soit $R$ un anneau. Une \emph{séquence} est une fonction $σ: ℕ_0 ⟶ R$. Nous écrirons $σ$ comme
\begin{displaymath}
  σ = [a_0,a_1,a_2, \dots ),
\end{displaymath}
où $a_i = σ(i)$. Nous désignons l'ensemble de toutes les séquences par $S$.  Nous identifions chaque élément $e ∈R$  comme l'élément
\begin{displaymath}
  [a,0,0, \dots) ∈S. 
\end{displaymath}
%
...
\end{displaymath}
Il est simple de montrer que $(S,+)$ est un groupe abélien, c'est-à-dire que (\ref{R1})-(\ref{R4}) tiennent. Par exemple (\ref{R1}) est montrée comme suit. Soient $ [a_0,a_1,a_2, \dots ), \, [b_0,b_1,b_2, \dots ) ∈ S$. On a
\begin{eqnarray}
  \label{eq:54}
...
\end{eqnarray}
Ici, \eqref{eq:56} découle de (\ref{R1}) dans l'anneau $R$.

L'opération de multiplication est la suivante. Étant donné deux séquences $[a_0,a_1,a_2, \dots ) ∈S$ et $   [b_0,b_1,b_2, \dots ) ∈S$, leur produit est 
\begin{displaymath}
...
\begin{displaymath}
  c_i = ∑_{j+k = i} a_jb_k.
\end{displaymath}
Il est simple de montrer que les conditions (\ref{R5}) et (\ref{R8}) sont satisfaites. L'élément unité $1 ∈ S$ est la séquence
\begin{displaymath}
  [1,0,0, \dots). 
\end{displaymath}
Cela montre que $S$ est un anneau. L'élément $x ∈ S$ spécifié dans le Théorème~\ref{thr:58} est la séquence
 \begin{displaymath}
 x =  [0,1,0, \dots). 
\end{displaymath}
...
\end{displaymath}
ce qui est la condition~\ref{item:29} du Théorème~\ref{thr:58}. 
...
\end{displaymath}
chaque $\mathbf{a}_i$ est de la forme $[a_i,0,0, \cdots )$ avec $a_i ∈R$, alors chaque $a_i$ doit être nul. Ceci conclut la preuve du Théorème~\ref{thr:58}.
