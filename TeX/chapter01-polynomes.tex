...
Un élément  $a ≠0$ de $R$ est un \emph{diviseur de zéro} s'il existe un $b≠0$ tel que $ab = 0$ ou $ba = 0$. Un anneau commutatif sans diviseurs de zéro et $1 ≠0$  est appelé \emph{anneau intègre}.  
...
\begin{example}
  \label{exe:32}
  \begin{enumerate}[i)]
  \item Les nombres entiers $ℤ$ avec l'addition et la multiplication standard forment un anneau commutatif sans diviseurs de zéro. Aussi $1 ≠ 0$. $ℤ$ est donc un anneau intègre. 
...
Le théorème suivant est trouvé dans les notes de cours du premier semestre~\cite{PMichel2026}.
\begin{theorem}
  \label{thr:58}
...
Le théorème suivant est démontré dans le cours \emph{anneaux et corps}.   Il nous donne une manière formelle d'introduire le concept d'une \emph{indéterminée} (ou  \emph{variable}) qui n'est autre que ledit élément $x ∈ S \setminus R$.
\begin{theorem}
  \label{thr:50}
  Soit $R$ un anneau, alors il existe un anneau $R' ⊇R$ ($R$ est un
  sous-anneau de $R'$) satisfaisant $1_{R'} = 1_R$ et qui possède un élément
  $x ∈ R \setminus R$ tel que
  \begin{enumerate}[(i)]
...
\begin{example}[Multiplication de polynômes] 
  \label{exe:33}
  \begin{eqnarray*}
    f(x) & = &  3 \, x^{3} + x + 2 ∈ ℤ[x]  \\
    g(x) & = & 2 \, x^{4} + 2 \, x^{2} + 1  ∈ ℤ[x] \\
    f(x) ⋅g(x) & = & 6 \, x^{7} + 8 \, x^{5} + 4 \, x^{4} + 5 \, x^{3} + 4 \,  x^{2} + x + 2   ∈ ℤ[x] \\ 
  \end{eqnarray*}
\end{example}
...
\begin{example}
  ...
  $f_q:\, K →K$ sont les mêmes applications car pour tout
...
Clairement 
\begin{displaymath}
  \deg\left( f(x) - a_n b^{-1}_m  x^{n-m} g(x) \right) < \deg(f(x))
\end{displaymath}
...
\begin{eqnarray*}
  f(x) & = &  \left(q_0(x) + {a_n}{ b_m }^{-1} x^{n-m} \right) g(x) + r(x) \\
       & = & = q(x)g(x) + r(x),
\end{eqnarray*}
 où $q(x) = q_0(x) + {a_n}{ b_m }^{-1} x^{n-m}$. 
%
...
\begin{algorithm}
  ...
  \If{$n < m$}
    \State\Return{$0$,$f(x)$}
    \Else
    \State{Soient $a,b ∈R$ coefficients dominants de $f$ et $g$ respectivement}  
...
Le Théorème suivant est une variante du Théorème~\ref{thr:33} dans laquelle le \emph{quotient} $q(x)$ est multiplié à droite, c.à.d. on exige
\begin{displaymath}
  f(x) = g(x) q(x) + r(x).  
\end{displaymath}
...
\begin{exercise}
  \label{exe:61}
  Modifier la démonstration du Théorème~\ref{thr:33} pour obtenir une démonstration du Théorème~\ref{thr:33-v}. 
\end{exercise}


\begin{example}
  \label{exe:60-right-left-div}
  C'est exemple montre que les quotients et les restes peuvent être divers dans les cas où le quotient est multiplié à droite et à gauche. 
Soit $ℤ^{2 ×2}$ l'anneau des matrices $2×2$ sur les entiers et 
\begin{eqnarray*} 
...
\end{eqnarray*}
deux polynômes.
Division avec reste (Théorème~\ref{thr:33}) nous donne $f(x) = q(x) ⋅g(x) + r(x)$ avec
...
\end{array}\right)
\end{displaymath}
Si le quotient $q(x)$ doit être multiplié à $g(x)$ à droite (Théorème~\ref{thr:33-v}), on obtient
$f(x) = g(x) ⋅ q(x) + r(x)$ avec
\begin{displaymath}
  q(x) =  \left(\begin{array}{rr}
1 & 0 \\
0 & 0
...
% \marginnote{Tout matériel sur Python est supplémentaire et ne compte pas vers l'examen.}[0cm] 

% Voici une implémentation en Python. Nous utilisons une liste Python pour re\-pré\-senter un polynôme. Par exemple, $f(x) = x^3 + 2x + 5$ serait représenté par la liste $[5,2,0,1]$. Ce code fonctionne pour de telles listes du type de données \textsc{Fraction} et lorsque l'élément de liste d'indice le plus élevé est non nul dans les deux listes. Si vous êtes intéressé, vous pouvez jouer avec ce code. Un exemple sur la façon de l'utiliser se trouve dans le dépôt Git~\cite{LaGIT}.
...
\begin{proposition}
  \label{prop:9}
  Soient $f ,g ∈ K[x]$ non tous les deux nul et soit  $d ∈ K[x]$ un polynôme satisfaisant les conditions \ref{item:24}) et \ref{item:25}) du Théorème~\ref{thr:55}.  
  Le polynôme $d$ est unique à multiplication près par une constante $\alpha \neq 0$.  Si $d$ est unitaire, alors $d$ est unique.  \label{item:26}
\end{proposition}
\begin{proof}
  Soient $d$ et $d'$ deux polynômes  satisfaisant \ref{item:24}) et \ref{item:25}). C.à.d. $d$ et $d'$ sont des diviseurs commun de $f$ et $g$ et chaque diviseur commun de $f$ et $g$ divise $d$ et $d'$.
  Les conditions impliquent   $d \mid d'$ et $d' \mid d$. 
   Alors il existe $z,z' ∈K[x]$ tel que $d = d' z'$ et $d' = dz$. Par suite, $d = d ⋅z ⋅ z'$. Le theorème~\ref{thr:34} ($d,d' ≠0$) implique que $z,z' ∈ K$.

   \medskip
   
\noindent    
 Si  $d$ et $d'$ sont unitaires, alors  $z=z'=1$, ce que démontre \ref{item:26}). 
\end{proof}
...
Pour calculer le plus grand diviseur commun de $f(x)$ et $g(x)$  on peut utiliser l'algorithme d'Euclide. Soient $f_0,f_1 ∈K[x]$  et  pas tous les deux nuls et $\deg(f_0) ≥ \deg(f_1)$. Si $f_1 = 0$, alors
\begin{displaymath}
\gcd(f_0,f_1) =   a^{-1}f_0, 
\end{displaymath}
où $a$ est le coefficient dominant de $f_0$. 
...
\begin{equation}
  ...
  f(x) = a ⋅ ∏_i p_i(x),
\end{equation}
dont les $p_i$ sont irréductibles et unitaires et $a ∈ K$. On va voir maintenant que cette factorisation est unique. 
...
\begin{enumerate}
\item    Démontre le Corollaire~\ref{co:11}.
\item Soit $K$ un corps fini, $ n=|K|$. \label{item:34} 
  \begin{enumerate}[a)] 
  \item Chaque $a ∈ K^*$ satisfait $a^{n-1} = 1$. \emph{Indice: Thm. de Lagrange}
  \item Le polynôme  $f(x) = x^n - x +1 ∈ K[x]$ ne possède pas de racines. 
  \item Conclure qu'il existent des polynômes irréductibles en $K[x]$ de degré aux moins $2$.  \emph{Indice: Factorisation de $f(x)$ en irréductibles. }
  \end{enumerate}
\end{enumerate}
...
\section{Le polynôme minimal d'une matrice}
\label{sec:le-polynome-minimal-1}


Soit $A ∈ K^{n ×n}$ une matrice et
\begin{displaymath}
  f(x) = a_0 + a_1 x + \cdots + a_n x^n∈ K[x] 
\end{displaymath}
un polynôme sur $K$.   \emph{L'évaluation} de $f(x)$ en $A$  est la matrice
\begin{displaymath}
  f(A) = a_0 I_n + a_1 A + \cdots + a_n A^n∈ K[x].  
\end{displaymath}

\begin{exercise}
  Pour $f(x), g(x) ∈ K[x]$  on a
  \begin{enumerate}[i)] 
  \item $(f+g)(A) = f(A) + g(A)$, 
...
Un polynôme $f(x) ∈ K[x]$ \emph{annule} $A$, si $f(A)$ est la matrice zéro
\begin{displaymath}
  f(A) = 0 ∈ K^{n ×n}. 
\end{displaymath}
...
\begin{theorem}
\label{thr:61}
  Soit $p(x) ∈ K[x]\setminus \{0\}$ un polynôme de degré minimal parmi les polynômes qui annulent $A$. 
  Alors $p(x)$ divise chaque polynôme  $f(x) ∈ K[x]$  tel que $f(A) = 0$.
Le polynôme $p$ est unique à multiplication prés par une constante $α ≠0$.
  Si $p$ est unitaire, alors $p$ est unique. 
\end{theorem}

\begin{proof}
  Supposons que $f(x)∈ K[x]$ annule $A$
  \begin{displaymath}
    f(A) = 0. 
  \end{displaymath}
On effectue division avec reste
  \begin{displaymath}
    f = q ⋅ p + r \text{ où } \deg(r) < \deg(p). 
  \end{displaymath}
  Parce que $r(A) = 0$ on a forcement $r = 0$. ($p$ est de degré minimal parmi les polynômes  évaluant à zéro). Alors $p \mid f$.

Comme dans la démonstration de la Proposition~\ref{prop:9} on montre que  $p$ est unique à multiplication prés par une constante $α ≠0$. Alors si $p$ est unitaire,  $p$ est unique. 
\end{proof}
...
\begin{definition}
\label{def:67}
Le polynôme \emph{unitaire} $p(x) ∈ K[x]\setminus \{0\}$ du Théorème~\ref{lem:31} est appelé le \emph{polynôme minimal} de $A ∈ K^{n ×n}$.
\end{definition}

\subsubsection{Comment trouver  le polynôme minimal?}
\label{sec:an-algor-aleat}

Soit $A  ∈ K^{ n ×n}$. 
Le théorème de Cayley-Hamilton (Théorème~\ref{thr:47}) affirme que $f_A(A) = 0$, où $f_A \in K[x]$ est le \emph{polynôme caractéristique} de $A$. Le degré de $f_A$ est $n$. 
  Une façon de calculer le polynôme minimal consiste à chercher le plus petit $d \in \mathbb{N}_+$, $d \leq n$ tel que
  \begin{displaymath}
    I = A^0, A, A^1, \cdots, A^d
  \end{displaymath}
  soient linéairement dépendants et de trouver $a_0,a_1, \dots, a_d$ tel que
  \begin{equation}
    \label{eq:60}
     a_0 I +  a_1 A + a_2 A^2+ \dots + a_d A^d = 0.
   \end{equation}
Le polynôme minimal de $A$ est $p(x) = a_0 + a_1x + \cdots + a_n x^n$. 
Ce $d$ aussi bien les coefficients $a_0,\dots,a_d$ de $p(x)$ se laissent calculé par l'algorithme de Gauss sur la matrice
\begin{eqnarray}
  \label{eq:61}
  B = \left(c_0,\dots,c_n\right) ∈ K^{n^2 × (n+1)} 
\end{eqnarray}
dont la colonne $c_k ∈ K^{n^2}$ représente la matrice $A^k ∈ K^{n ×n}$ pour $k=0,\dots,n$. C'est à dire, la  $n ⋅ i + j$-ème composante  de  $c_k$ est 
\begin{equation}
  \label{eq:62}
  (A^k)_{ij}. 
\end{equation}
En fait, le premier vecteur ($j$ minimal) dans la liste de vecteurs de la base du $\ker(B)$, comme décrit en Chapitre~\ref{sec:limage-et-le}~\eqref{eq:b-20} nous donne le degré et les coefficients du polynôme minimal. 


La complexité de l'élimination de Gauss avec $m'$ lignes et $n'$ colonnes est
\begin{displaymath}
   Ω(m' \cdot n' \cdot \min\{m',n'\})
\end{displaymath}
opérations arithmétique du corps $K$. Dans notre cas, cela équivaut à $Ω(n^4)$. Cependant, il existe un algorithme de calcul du polynôme minimal qui effectue   $O(n^3)$ opérations arithmétiques du corps $K$. Cet algorithme est aléatoire, c'est à dire, que cet algorithme fais des choix basé sur des expériments de probabilité. À la fin, l'algorithme retourne  un polynôme $p(x) ∈ K[x]$. Ce polynôme est \emph{probablement} le polynôme minimal de $A ∈ K^{n ×n}$. En fait, la probabilité de l'évènement
\begin{displaymath}
...
Soit $v \in K^n \setminus\{0\}$. On montre de manière similaire à la démonstration du Théorème~\ref{thr:61} la propriété suivante.

\begin{theorem}
  \label{thr:64}
  Soit  $p_v(x) \in K[x] \setminus \{0\}$ de degré minimal  tel que
\begin{equation}
  \label{eq:2}
    p_v(A)\, v = 0
  \end{equation}
  soit vérifiée. Ce polynôme $p_v(x)$ divise tout polynôme $h(x) \in K[x]$ tel que $h(A)v = 0$.
Le polynôme $p_v$ est unique à multiplication prés par une constante $α ≠0$.
  Si $p$ est unitaire, alors $p$ est unique. 
\end{theorem}

Le polynôme $p_v(x)$ est appelé l'\emph{$A$-annulateur de $v$}.

Le démonstration est comme celle du Théorème~\ref{thr:61}.  Soit $h(x) \in K[x]$ un polynôme tel que $h(A)v = 0$. Si $p_v(x)$ ne divise pas $h(x)$, alors, par division euclidienne comme ci-dessus, on obtient
  \begin{displaymath}
    r(A) = h(A) - q(A) p_v(A) ∈ K^{n ×n}
  \end{displaymath}
  et par conséquent $r(A)\,v = 0$, ce qui contredit la minimalité du degré de $p_A^v(x)$. Alors $p_v(x)$ divise tout polynôme $h(x) \in K[x]$ tel que $h(A)v = 0$. Par conséquence 
 $p_v$ est unique à multiplication prés par une constante $α ≠0$. 
 Alors  si $p$ est unitaire,  $p$ est unique. 

  \subsubsection{Algorithme aléatoire}
  ...
  \begin{displaymath}
    I \,v, A\,v, A^2\,v, \cdots, A^i\,v \quad \text{soient linéairement dépendants.}
  \end{displaymath}
  ...
  \begin{equation}
    \label{eq:63}
    C = \left(  I \,v, A\,v, A^2\,v, \cdots, A^n\,v \right) ∈ K^{n × (n+1)}. 
  \end{equation}
  Le calcul de $I \,v, A\,v, A^2\,v, \cdots, A^n \,v$ nécessite $O(n^3)$ opérations dans le corps. Ce sont des produits matrice-vecteur.
  Aussi l'algorithme de Gauss effectue 
  $O(n^3)$ opérations.  


  
  \subsubsection*{Probabilité d'échec}
  
   \begin{theorem}
     \label{thr:63}
     La probabilité de l'événement $p_v(x) \neq p(x)$ est au plus $n/|S|$.
   \end{theorem}
   \begin{proof}
     Soit $p(x) = \ell_1(x)^{e_1} \cdots \ell_k(x)^{e_k}$ la
     factorisation unique de $p(x)$ en facteurs irréductibles
     distincts. On remarque que $k \leq \deg(p(x)) \leq n$. Si
     $p_v(x)$ n'est pas égal à $p(x)$, alors il doit exister un
     facteur irréductible, disons $\ell_1$, tel que
     $(m_A/ \ell_1)(A) v =0$ soit vérifié. Considérons maintenant
     $\ell_1$. Puisque $(m_A/ \ell_1)(A) \neq 0$, on a clairement
     \begin{displaymath}
       P \left[ (m_A/ \ell_1)(A) v = 0 \right]  \leq 1/|S|.
     \end{displaymath}
     La borne annoncée découle alors de l'application de l'inégalité de Boole (union bound).
   \end{proof}


Si l'ensemble $S$ est de cardinalité $|S| = 2 n^2$, la probabilité de l'événement $p_v(x) \neq p(x)$ est au plus $1/2$. Avec une telle choix de $S$, si l'algorithme est répété $k$-fois, la probabilité de l'évènement
\begin{displaymath}
  P \left[ \text{ aucun } p_v  \text{ est égal à } p \right]  \leq 2^{-k}. 
\end{displaymath}
Pour $k = 100$, par exemple, cette probabilité est inférieure aux réciproque des particules élémentaires de l'univers et en peut argumenter que cette probabilité es négligeable. 

\begin{algorithm}%[Élimination de Gauss]
\label{alg:5}
  \caption{Polynôme minimal} 
\begin{tabbing}
\textbf{Entrée:} $A ∈ K^{m ×n}$, $k ∈ ℕ_+$, $S ⊆ K$ t.q. $|S| = 2n^2$ \\
\textbf{Sortie:} \= $p(x) ∈ K[x]$  \\
\pushtabs
\textbf{Répète}\= $100$ fois: \\
\> Choisir $v ∈ S^n$ aléatoirement\\
\> Calculer $p_v(x)$\\
\textbf{ retour:} Polynôme $p_v(x)$ de degré maximal.
\poptabs
\end{tabbing}
\end{algorithm}
...
\begin{theorem}
  \label{thr:62}
  Soit $A ∈ K^{n ×n}$ et $k ∈ n$. L'algorithme~\ref{alg:5} effectue $O(n^3)$ opérations de corps. La probabilité de l'évènement que le polynôme retourné n'est pas le polynôme minimal de $A$ est au plus $2^{-100}$. 
\end{theorem}
   ...
   \subsection*{Exercices}
   \begin{enumerate}
   \item Soit $V$ un espace vectoriel de dimension finie sur le corps $K$ et $T: V \longrightarrow V$ un endomorphisme.
     \begin{enumerate}[i)]
     \item        Montrer que l'ensemble $I = \{ f(x) \in K[x] \mid f(T) = 0\}$ est un idéal de $K[x]$ et qu'il est engendré par le polynôme minimal de $T$.      \label{item:1}
     \item   Soit $v \in V$. Montrer que l'ensemble $I = \{ f(x) \in K[x] \mid f(T)(v) = 0\}$ est un idéal de $K[x]$ et que celui-ci est engendré par $p_T^v(x)$, le $T$-annulateur de $v$. \label{item:2}
     \item Montrer que $m_T(x)$ est le plus petit commun multiple (des) polynômes $p_T^v(x), \, v \in V$ (en nombre fini). \label{item:3}     
     \end{enumerate}
   \item Soit $A ∈ K^{n ×n}$ la matrice
     \begin{displaymath}
       A =
       \begin{pmatrix}
         0 & 0 & \cdots    & 0 & a_0 \\
     ...
     \end{displaymath}
     \begin{enumerate}[i)]  
     \item Quel est le polynôme minimal $p_{e_1}(x)∈ K[x]$ du vecteur $e_1 ∈ K^n$?
     \item Quel est le polynôme minimal $p(x)∈ K[x]$ de la matrice $A$?
       \emph{ Indice: Thm. de Cayleigh-Hamilton et Théorème~\ref{thr:64}. } 
     \end{enumerate}
...
Dans cette section, nous fournirons une preuve du Théorème~\ref{thr:50}. Soit $R$ un anneau. Une \emph{suite} est une fonction $σ: ℕ_0 ⟶ R$. Nous écrirons $σ$ comme
\begin{displaymath}
  σ = [a_0,a_1,a_2, \dots ),
\end{displaymath}
...
L'opération de multiplication est la suivante. Étant donné deux suites $[a_0,a_1,a_2, \dots ) ∈S$ et $   [b_0,b_1,b_2, \dots ) ∈S$, leur produit est 
\begin{displaymath}
