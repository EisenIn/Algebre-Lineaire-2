...
\begin{eqnarray}
  \label{eq:61}
  B = \left(c_0,\dots,c_n) ∈ K^{n^2 × (n+1)} \right)
\end{eqnarray}
...
La complexité de l'élimination de Gauss avec $m'$ lignes et $n'$ colonnes est $ Ω(m' \cdot n' \cdot \min\{m',n'\})$ opérations arithmétique du corps $K$. Dans notre cas, cela équivaut à $Ω(n^4)$. Cependant, cela peut être fait en  $O(n^3)$ opérations arithmétiques du corps $K$ comme nous allons le montrer maintenant.
...
\begin{theorem}
  \label{thr:62}
  Soit $A ∈ K^{n ×n}$ et $k ∈ n$. L'algorithme~\ref{alg:5} effectue $O(n^3)$ opérations de corps. La probabilité de l'évènement que le polynôme retourné n'est pas le polynôme minimal de $A$ est au plus $1/2^{-100}$. 
\end{theorem}
