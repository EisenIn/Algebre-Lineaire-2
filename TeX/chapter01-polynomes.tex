...
Soit $v \in K^n \setminus\{0\}$. On montre de manière similaire à la démonstration du Théorème~\ref{thr:61} la propriété suivante. Il existe un  polynôme unitaire unique $p_v(x) \in K[x] \setminus \{0\}$ de degré minimal  tel que
\begin{equation}
  \label{eq:2}
    p_v(A)\, v = 0
  \end{equation}
  soit vérifiée. Ce polynôme $p_v(x)$ divise tout polynôme $h(x) \in K[x]$ tel que $h(A)v = 0$. Le polynôme $p_v(x)$ est appelé l'\emph{$A$-annulateur de $v$}.


  Voyons pourquoi cela tient. Soit $h(x) \in K[x]$ un polynôme tel que $h(A)v = 0$. Si $p^v_A(x)$ ne divise pas $h(x)$, alors, par division euclidienne comme ci-dessus, on obtient
  \begin{displaymath}
    r(A) = h(A) - q(A) p_v(A)
  \end{displaymath}
  et par conséquent $r(A)\,v = 0$, ce qui contredit la minimalité du degré de $p_A^v(x)$.
...
\begin{displaymath}
  P \left[ \text{ aucun } p_v = p \right]  \leq 2^{-k}. 
\end{displaymath}
