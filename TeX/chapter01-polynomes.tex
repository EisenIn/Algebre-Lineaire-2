...
On peut formuler une variante du Théorème~\ref{thr:33} dans laquelle le \emph{quotient} $q(x)$ est multiplié à droite, c.à.d. on exige
\begin{displaymath}
  f(x) = g(x) q(x) + r(x).  
\end{displaymath}
...
\begin{example}
  ...
  C'est exemple montre que les quotients et les restes peuvent être divers dals les cas où le quotient est multiplié à droite et à gauche. 
...
\end{displaymath}
deux polynômes.
Division avec reste nous donne $f(x) = q(x) ⋅g(x) + r(x)$ avec
...
Si le quotient $q(x)$ doit être multiplié à $g(x)$ à droite, on obtient
