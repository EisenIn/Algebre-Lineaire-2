...
Le théorème de Cayley-Hamilton~\cite{PMichel2026} affirme que $f_A(A) = 0$, où $f_A \in K[x]$ est le \emph{polynôme caractéristique} de $A$.
  Une façon de calculer le polynôme minimal consiste à chercher le plus petit $i \in \mathbb{N}_+$, $i \leq n$ tel que
  \begin{displaymath}
    I = A^0, A, A^1, \cdots, A^i
  \end{displaymath}
  soient linéairement dépendants. Chaque vérification de ce type peut être effectuée par élimination de Gauss. La complexité de l'élimination de Gauss avec $m'$ lignes et $n'$ colonnes est $ Ω(m' \cdot n' \cdot \min\{m',n'\})$ opérations arithmétique du corps $K$. Dans notre cas, cela équivaut à $i \cdot n^2 \cdot i$. Grâce à des mises à jour itératives, le plus petit $i$ et $m_A(x)$ peuvent être trouvés en un temps $O(n^2 d^2)$, où $d$ est le degré de $m_A(x)$. Cependant, cela peut être fait en un temps $O(n^2 d)$ comme nous allons le montrer maintenant.
  ...
  L'algorithme est le suivant. Fixons un ensemble fini $S \subseteq K$ avec $|S| = M$ et choisissons $v \in K^n$ aléatoirement en sélectionnant chaque composante de $v$ uniformément dans $S$. Ensuite, calculons $p_v(x)$ en trouvant le plus petit $i$ tel que
  \begin{displaymath}
...
%
   Le temps d'exécution de l'algorithme peut être estimé comme suit.
   \begin{itemize}
   \item Le calcul de $I \,v, A\,v, A^2\,v, \cdots, A^d \,v$ nécessite $O(n^2 d)$ opérations dans le corps. Ce sont des produits matrice-vecteur. Ici, $d$ est le degré de $m_A(x)$.
   \item Vérifier si $ I \,v, A\,v, A^2\,v, \cdots, A^i\,v$ est linéairement indépendant nécessite $O(n i^2)$ opérations. C'est l'élimination de Gauss.
     \item Le plus petit $i$ est trouvé par une méthode de Gauss mise à jour itérative en un temps $O(n d^2)$.
     \end{itemize}
     Cela représente un total de $O(n^2 d)$ opérations dans le corps.
