...
Par contre, si $K$ est un corps infini, deux polynômes différents induises  deux applications différentes. Nous allons voir les détails de ce fait maintenant.
...
\begin{theorem}
  \label{thr:54}
  Soient $f(x)$ et $g(x)$ deux polynômes sur $K$ pas tous les deux égal à zero et soit 
  \begin{displaymath}
    I = \{ u ⋅ f + v ⋅ g ： u,v ∈ K[x]\}.
  \end{displaymath}
...
\begin{proof}
  On observe que $I$ contient des polynômes non nul. Soit $d ∈ I \setminus \{0\}$ de degré minimal et $u',v' ∈ K[x]$ tel que
  \begin{displaymath}
    u' ⋅ f + v' ⋅g = d. 
  \end{displaymath}
  Soit $u⋅f + v⋅ g ∈ I$. Si $d$ ne divise pas ce polynôme, division avec reste donne $u⋅f + v⋅ g = qd +r$, où $r≠0$  et $\deg(r) < \deg(d)$. Alors
  \begin{displaymath}
    r = (u - qu') ⋅ f + (v - qv') ⋅ g ∈ I
  \end{displaymath}
  est c'est une contradiction du fait que $d ∈ I \setminus\{0\}$ de degré minimal. C'est à dire $d \mid u⋅f + v⋅ g$ et alors il existe un $h∈K[x]$ tel que $h⋅d =  u⋅f + v⋅ g$. Des que $h⋅d ∈I$ pour tous $h ∈K[x]$ on a démontré l'assertion.   
\end{proof}


\begin{definition}
  \label{def:54}
  Un polynôme $f(x) ∈ K[x] \setminus \{0\}$  dont le coefficient dominant est $1$ est appelé un \emph{polynôme unitaire}. 
\end{definition}


\begin{definition}
  \label{def:55}
  Soient $f,g ∈ K[x]$ pas tous les deux égal à zéro. Un diviseur commun de $f$ et $g$  est un diviseur de $f$ et $g$. 
\end{definition}
...
\begin{proof}
  L'assertion \ref{item:24}) suit du fait que $f,g ∈ I$. Soient $u,v ∈K[x]$  tel que
  $    d = u⋅f  + v ⋅ g $
  et soit $w$ un diviseur commun de $f$ et $g$. Alors il existent $f',g' ∈K[x]$ tel que  $f = f' w$ et $g = g' w$. Alors
  \begin{displaymath}
    d = (u⋅f'  + v ⋅ g') w,
  \end{displaymath}
  ce que montre que $w \mid d$ et \ref{item:25}). 
  Soient $d$ et $d'$ deux polynômes unitaires satisfaisant~\eqref{eq:51}. \ref{item:24}) et \ref{item:25}) impliquent $d \mid d'$ et $d' \mid d$.
  Alors il existent $z,z' ∈K[x]$ tel que $d = d' z'$ et $d' = dz$. Alors $d = d ⋅z ⋅ z'$. Theorème~\ref{thr:34} implique $z,z' ∈ K$. Et des que $d$ et $d'$ sont unitaires, $z=z'=1$, ce que démontre \ref{item:26}). 
\end{proof}

\begin{definition}
  \label{def:56}
  L'unique polynôme unitaire $d ∈ K[x]$ satisfaisant \eqref{eq:51} est appelé  le \emph{plus grand commun diviseur} de et $f$ et $g$. 
\end{definition}

\subsection{L'algorithme de Euclide}
\label{sec:lalg-de-eucl}



Pour calculer le plus grand diviseur commun de $f(x)$ et $g(x)$  on peut utiliser l'algorithme d'Euclide. Soient $f_0,f_1 ∈K[x]$ pas tout les deux nuls et $\deg(f_0) ≥ \deg(f_1)$. Si $f_1 = 0$, alors
\begin{displaymath}
\gcd(f_0,f_1) =   f_0. 
\end{displaymath}
...
\end{displaymath}
est le résultat de la division avec reste de $f_{i-1} $ par $f_i$. 
...
\begin{displaymath}
  ...
  f_4 =  0. 
\end{displaymath}
Alors tout diviseur commun de $f_0$ et $f_1$ divise $f_3 =  3 x^{2} + 1$ et $f_3$ est aussi un diviseur commun. On divise par $3$ (multiplie avec $2$) et on obtient  $\gcd(f_0,f_1) = x^2 +2$. 
...
\begin{equation}
  ...
  f(x) = a ⋅ ∏_i p_i(x),
\end{equation}
dont les $p_i$ sont irréductibles est unitaires et $a ∈ K$. Est-ce que cette factorisation est unique?
...
\begin{proof}
  Par induction il suffit de démontrer l'assertion pour $k=2$. Alors, supposons $p \mid fg$, $f,g ∈K[x] \setminus \{0\}$. Si $p$ ne divise pas $f$, alors $\gcd(p,f) = 1$. Soient $u,v ∈ K[x]$ t.q.  $up + vf = 1$. Alors $upg + vfg = g$, alors $u \mid g$. 
\end{proof}


\begin{theorem}
  \label{thr:57}
  La factorisation~\eqref{eq:33} est unique sauf à l'ordre des $p_i$. 
\end{theorem}

\begin{proof}
  Pour une  factorisation $f(x) = a ∏_i q_i(x)$, où les $q_i$ sont irréductibles et unitaires on utilise Théorème~\ref{thr:56} pour déduire qu'il  existe $j$ tel que  $p_1 \mid q_j$. Des que $p_1$ et $q_j$ sont  irréductibles et unitaires, alors $p_1 = q_j$. On divise par $p_1$. L'assertion suit par induction. 
\end{proof}
