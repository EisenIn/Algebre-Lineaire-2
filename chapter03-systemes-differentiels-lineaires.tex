...
\end{theorem}
\begin{proof}
  La formule~\eqref{eq:21} révèle que $\deg(f\cdot g) \leq \deg(f) + \det(g)$. 
...
\begin{theorem}
  \label{thr:33}
  Soient $f,g \in K[x]$ et $\deg(g) >0$. Il existe $q,r \in K[x]$ tels que 
  \begin{displaymath}
    f(x) = q(x) g(x) + r(x) 
  \end{displaymath}
  et $\deg(r) < \deg(g)$. 
...
\begin{theorem}
  \label{thr:40}
  Un polynôme  $f(x) \in K[x]$ a une factorisation 
  \begin{displaymath}
    f(x) = a^* \prod_j p_j(x)
  \end{displaymath}
...
\begin{definition}
  ...
  \begin{displaymath}
    f(A) = a_n A^n + a_{n-1}A^{n-1}+ \cdots + a_1 A + a_0 \cdot I_n \in K^{n \times n}. 
  \end{displaymath}
\end{definition}
...
\begin{definition}
  \label{def:35}
  Soit $A:V \rightarrow V$ un endomorphisme et $W \subseteq C$ un sous-espace de $V$. On dit que $W$ est \emph{invariant sous $A$} si $A(x) \in W$ pour tout $x \in W$. 
\end{definition}
...
Une matrice $A \in \C^{n \times n}$ est en \emph{forme normale de Jordan} si $A$ est en forme bloc diagonale, où tout les blocs sur la diagonale sont des blocs Jordan, alors une matrice de la forme 
\begin{displaymath}
  A =
  \begin{pmatrix}
    B_1 \\
  ...
  \end{pmatrix}
\end{displaymath}
où les matrices $B_j \in \C^{n_j\times n_j}$ sont des blocs Jordan. 
\end{definition}
...
\begin{definition}
  ...
  \begin{displaymath}
    ...
    \end{pmatrix}
     ...
     \end{pmatrix}. 
  \end{displaymath}
  Le décalage plus une constante est une application linéaire 
  \begin{displaymath}
    U + \lambda \cdot I. 
  \end{displaymath}
\end{definition}

C'est facile à voir que la matrice représentant le décalage plus $λ$ est 
un seul bloc Jordan 
\begin{displaymath}
 \begin{pmatrix}
      λ & 1 \\
...
\begin{proof}
  Des que l'espace des applications linéaires sur $V$ est un espace vectoriel sur $\C$ de dimension finie il existe un $k \in \N$ tel que 
  \begin{displaymath}
    I, \, T, \, T^2, \dots, T^k
  \end{displaymath}
sont linéairement dépendants. Alors il existe un polynôme $p(x) \in \C[x] \setminus \{ 0 \}$ tel que $p(T) = 0$. Le théorème fondamental d'algèbre implique que 
\begin{displaymath}
  p(x) = ( x - λ_1)^{m_1} \cdots ( x - λ_k)^{m_k} 
\end{displaymath}
avec des   $λ_i$ différentes. 
...
Rappel: Si  $\phi_{\mathscr{B}}$ est l'ismomorphisme $\phi_{\mathscr{B}} \colon V \longrightarrow \C^n$, où $\phi_{\mathscr{B}}(x) = [x]_{\mathscr{B}}$ sont les coordonnées de $x$ par rapport à la base ${\mathscr{B}}$,  on a le diagramme suivant 
\begin{displaymath}
  {
  \begin{CD}
    V     @>T>>  V\\
...
\begin{remark}
  \label{rem:4}
  Si on inverse l'ordre de la base $ℬ$ alors si on liste les éléments de droite à gauche on obtient une base $ℬ'$ et la matrice $A_{ℬ'}^{N }$ de l'application $N$  a la forme 
  \begin{displaymath}
    A_{ℬ'}^N =
    \begin{pmatrix}
      J_1 \\
...
\end{displaymath}
Par conséquence $N+ λI$ est représentée par 
\begin{displaymath}
...
\begin{proof}[Démonstration du Théorème~\ref{thr:38}] 
  ...
  En concaténant les orbites des éléments d'une base, nous obtiendrons un ensemble de vecteurs qui engendre $V$. 
  Supposons alors que nous avons un ensemble $x_1,\dots,x_\ell$ dont les orbites 
  \begin{equation}
    \label{eq:27}
    x_1,Nx_1,\dots,N^{m_1-1}x_1, \,\dots \, ,  x_\ell,Nx_\ell,\dots,N^{m_\ell-1}x_\ell
  \end{equation}
  engendrent $V$. Ici $m_i$ est la durée de vie de $x_i$. Si~\eqref{eq:27} est linéairement dépendant nous allons remplacer un $x_i$ par un vecteur $y$ tel que 
  \begin{enumerate}[i)]
  \item Les orbites de $x_1,\dots, x_{i-1},y,x_{i+1},\dots,x_n$ engendrent ensemble  $V$ aussi, 
  \item la somme des durées de vie de $x_1,\dots, x_{i-1},y,x_{i+1},\dots,x_n$  est strictement plus petite que la somme des durées de vie de $x_1,\dots,x_\ell$. 
  \end{enumerate}
Comme ça nous avons montré le théorème parce que un tel procédé doit se terminer. 
...
\end{displaymath}
Maintenant, nous allons appliquer l'application $N$ $k$-fois où $k$ est le dernier moment, où pas tous les termes 
\begin{displaymath}
  β_i^j N^{k+i}x 
\end{displaymath}
sont égaux à zéro. Comme ça nous avons trouvé un sous-ensemble $J ⊆ \{1,\dots,k\}$ et des $γ_j ≠ 0$ tels que 
\begin{displaymath}
  \sum_{j \in J} γ_j N^{m_j-1}x_j = 0.
\end{displaymath}
Soit $m = \min_{j \in J} {m_j-1}$ et soit $i \in J$ un index où le minimum est atteint. Alors 
\begin{displaymath}
 0 =  N^m  \sum_{j \in J} γ_j N^{m_j-1 - m}x_j  
\end{displaymath}
et $γ_i x_i$ est un terme dans la somme en dessus. 

Maintenant on remplace $x_i$ avec $y = \sum_{j \in J} γ_j N^{m_j-1 - m}x_j$. Il est facile de voir que les orbites de 
\begin{displaymath}
  x_1,\dots, x_{i-1},y,x_{i+1},\dots,x_n
\end{displaymath}
engendrent encore $V$. Et la durée de vie de $y$ est au plus $m<m_i$. On a alors démontré le théorème.  
\end{proof}
